...
def model(x_train,y_train,x_test,y_test):
    ...
    model.add(Flatten())
    model.add(Dense(int({{quniform(420,512,1)}}), kernel_initializer={{choice(['normal', 'he_normal','uniform','he_uniform'])}}, activation={{choice(['relu','linear'])}}))
    model.add(Dense(int({{quniform(320,420,1)}}), kernel_initializer={{choice(['normal', 'he_normal','uniform','he_uniform'])}}, activation={{choice(['relu','linear'])}}))
    model.add(Dense(int({{quniform(1,128,1)}}), kernel_initializer={{choice(['normal', 'he_normal','uniform','he_uniform'])}}, activation={{choice(['relu','linear'])}}))
    model.add(Dense(num_outputs, kernel_initializer={{choice(['normal', 'he_normal','uniform','he_uniform'])}},activation={{choice(['relu','linear'])}})) # final output has 7 dimensions
    # Print model summary
    model.summary()
    # Compile model
