...
int main(int argc, char** argv)
{
  ...
  Int_t    nselected = 0, selectedhit=0;
  ...
  vector<vector<float >>  hitpmt;
  vector<vector<float >>  selectedhitarray;
  vector <float> hitpmttmp;
  ...
  for (event = 0; event < n_events; event++)
          ...
          if(pmtinfo->GetType(id) == innerPMTcode)
            {
              ...
              hitpmttmp.push_back(pos[0]*0.1);    //x
              hitpmttmp.push_back(pos[1]*0.1);       //y
              hitpmttmp.push_back(pos[2]*0.1);       //z
              hitpmttmp.push_back(pmt->GetTime());   //t
              hitpmttmp.push_back(pmt->GetCharge()); //q
              innerPE += pmt->GetCharge();
	      hitpmt.push_back(hitpmttmp);
	      hitpmttmp.clear();
              innerHit++;
	    }
	...
	// check that the hits are from direct light
	// for the moment, assume the first hit is direct light
	// this is ok until we start to add in dark noise
	sort(hitpmt.begin(), hitpmt.end(), sortcol);
	float xi = hitpmt[0][0];
	float yi = hitpmt[0][1];
	float zi = hitpmt[0][2];
	float ti = hitpmt[0][3]*21.8;
	selectedhitarray.push_back(hitpmt[0]);
	selectedhit=0;
	for (int hit=1; hit<innerHit; hit++){
	  float xj = hitpmt[hit][0];
	  float yj = hitpmt[hit][1];
	  float zj = hitpmt[hit][2];
          float tj = hitpmt[hit][3]*21.8;
  	  float tdiff2 = pow((tj-ti),2);
          float distdiff2 = pow(xi-xj,2)+pow(yi-yj,2)+pow(zi-zj,2);
	  // only use hits from unscattered light
          if (tdiff2 < distdiff2){
	    selectedhitarray.push_back(hitpmt[hit]);
	    selectedhit++;
	    }
	  }
	nselected=selectedhit;
	hitpmt.clear();
	...
	for(int feature=0;feature<5;feature++)
          {
          for(hit=0; hit<nselected; hit++)
            {
            if (hit==nselected-1) hit_csvfile << selectedhitarray[hit][feature];
	    else hit_csvfile << selectedhitarray[hit][feature] << ",";
            }
          hit_csvfile << "\n";
	  }
	selectedhitarray.clear();
      } 
    }
