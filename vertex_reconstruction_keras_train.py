...
plt.hist(x_train_vtx[:,2][~np.isnan(x_train_vtx[:,2])],bins=500,histtype='step')
...
x_train_test = x_train_test.reshape(x_samples,num_features,max_hits)
print(x_train_test)
'''
...
x_train = x_train.reshape(x_samples,num_features,max_hits)
...
def model():
    ...
    model.add(Flatten())
    model.add(Dense(419, kernel_initializer='he_normal', activation='relu'))
    model.add(Dropout(0.43165026280345337))
    model.add(Dense(369, kernel_initializer= 'normal', activation='softmax'))
    model.add(Dropout(0.07237577863609031))
    model.add(Dense(150, kernel_initializer= 'he_normal', activation='linear'))
    model.add(Dropout(0.49904931629816485))
    model.add(Dense(num_outputs, kernel_initializer='he_uniform', activation='linear')) # final output has 3 dimensions
    # Print model summary
    model.summary()
    # Compile model
...
filepath = 'weights_no_noise_model2.hdf5'
