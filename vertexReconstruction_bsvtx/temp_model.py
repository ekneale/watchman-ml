...
bs_x_scaler_vtx = preprocessing.MinMaxScaler(feature_range=(-1,1))
...
bs_X = bs_x_scaler_vtx.fit_transform(bs_X.reshape(-1,bs_X.shape[-1])).reshape(bs_X.shape) 
...
bs_x_train,bs_x_test = train_test_split(bs_X,random_state=1)
...
def keras_fmin_fnct(space):
    ...
    hidden1 = Dense(int(space['int']), kernel_initializer=space['kernel_initializer'], activation=space['activation'])(flat1)
    hidden2 = Dense(int(space['int_1']), kernel_initializer=space['kernel_initializer_1'], activation=space['activation_1'])(hidden1)
    hidden3 = Dense(int(space['int_2']), kernel_initializer=space['kernel_initializer_2'], activation=space['activation_2'])(hidden2)
    merge = Concatenate()([hidden3,bonsai_vtx])
    output = Dense(num_outputs, kernel_initializer=space['kernel_initializer_3'],activation=space['activation_3'])(merge) # final output has 7 dimensions
    # Print model summary
...
#    tbCallBack = keras.callbacks.TensorBoard(log_dir='./logs/fit', histogram_freq=1)
    ...
    result = model.fit([x_train,bs_x_train],y_train, 
            batch_size=int(space['int_3']),
            epochs=int(space['int_4']),
            verbose=2,
            validation_data= ([x_test,bs_x_test],y_test),
            callbacks=[early_stopping,checkpointer])

    mse,acc = model.evaluate([x_test,bs_x_test],y_test,verbose=0)
